...
const drawMiniContour = vi.fn();
vi.mock('../../ui/renderers/drawMiniContour', () => ({
  drawMiniContour,
}));
...
describe('rangeUpdateService', () => {
  ...
  describe('updateNoteRange', () => {
    ...
    it('does not call drawMiniContour if canvas is absent', () => {
      context.container = null;
      updateNoteRange(context, ['A0', 'C9']);

      expect(drawMiniContour).not.toHaveBeenCalled();
    });

    it('warns and exits if pitchToMidi returns null', () => {
      const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
      updateNoteRange(context, ['Z9', 'C9']); // invalid low note
    ...
    it('warns and exits if lowMidi >= highMidi', () => {
      const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
      updateNoteRange(context, ['C9', 'A0']); // inverted range
