export function drawKeys(ctx, keyMap, highlightNotes = new Set(), keyToNoteMap = null) {
  ...
  for (const key of Object.values(keyMap)) {
    if (!key.isBlack) {
      ...
      // KeyChar (keyboard mapping) above note label
  ...
  for (const key of Object.values(keyMap)) {
    if (key.isBlack) {
      ...
      // KeyChar label above the black key
      if (noteToKey[key.note]) {
...
function drawRoundedKey(ctx, { x, width, height, fill, stroke, radius = 8 }) {
  ctx.beginPath();
  ctx.moveTo(x, 0);
